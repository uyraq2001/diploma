...
\begin{figure}[!ht]
    ...
    \caption{Сравнение поведения $\beta_i(t), MFI1_i(t)$ и $MFI2_i(t)$}\label{fig:MFI}
\end{figure}

Полный текст программы для проведения данного эксперимента представлен в приложении\,\ref{app:MFI}.

Также был проведён эксперимент для анализа динамики среднего индекса дружбы, $MFI1_i$ и $MFI2_i$ в сети модели Барабаши-Альберт. Результаты этого эксперимента также были усреднены по 10 попыткам. Графики изменений средних значений перечисленных величин можно увидеть на Рис. \ref{fig:meanMFI}.

\begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.8]{mean_MFI.png}
    \caption{Сравнение динамики изменения $\beta_i(t), MFI1_i(t)$ и $MFI2_i(t)$}\label{fig:meanMFI}
\end{figure}
...
Из графиков динамики средних значений $\beta_i(t), MFI1_i(t)$ и $MFI2_i(t)$ представленных на Рис.\,\ref{fig:meanMFI} можно cделать вывод, о том что в отличии от бесконечно растущего индекса дружбы, средние значения обеих предложенных метрик весьма быстро стабилизируются после короткого периода быстрого изменения среднего значения на начальных итерациях генерации графа. Это можно объяснить, тем что на начальных итерациях структура графа модели Барабаши"--~Альберт непредсказуема и претерпевает значительные изменения при каждом добавлении новой вершины, однако со временем модель стабилизируется и её поведение становится всё более и более предсказуемым. 

Также из Рис.\,\ref{fig:meanMFIlog}, на котором изображены результаты того же эксперимента на логарифмической шкале, можно заметить, что график $MFI1_i(t)$ представляет собой нисходящую прямую линию, что указывает на то что среднее значение данной метрики изменяется по обратному степенному закону.

\begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.6]{mean_MFI_log.png}
    \caption{Сравнение динамики изменения $\beta_i(t), MFI1_i(t)$ и $MFI2_i(t)$ на логарифмической шкале}\label{fig:meanMFIlog}
\end{figure}
